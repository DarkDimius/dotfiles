genrule(
    ...
    cmd = select({
                           ...
                           "@me_dd_zap//tools/config:darwin": # os x bazel has wrapper scripts around CC, and makes does not link shebangs
                                      "export WORKSPACE_TOP=$$(pwd)/ && \
...
cc_library(
    ...
    linkopts = select({
                       "@me_dd_zap//tools/config:linux": ["-ldl"], # side step https://github.com/jemalloc/jemalloc/issues/948
                       "//conditions:default": [],
                   }),
)
